...
syn match   pythonDecorator    '^\s*\zs@\ze\h' display nextgroup=pythonDottedName skipwhite
if s:Python2Syntax()
...
if s:Enabled('g:python_highlight_string_formatting')
  ...
  if s:Python2Syntax()
    ...
    syn match pythonStrFormatting '%\%(([^)]\+)\)\=[-#0 +]*\d*\%(\.\d\+\)\=[diouxXeEfFgGcrs%]' contained containedin=pythonString,pythonRawString,pythonBytesContent
    syn match pythonStrFormatting '%[-#0 +]*\%(\*\|\d\+\)\=\%(\.\%(\*\|\d\+\)\)\=[diouxXeEfFgGcrs%]' contained containedin=pythonString,pythonRawString,pythonBytesContent
  endif
endif
